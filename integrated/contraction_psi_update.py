...
class ContractionPsiUpdate:
    ...
    def simulate_sequence(
        ...
        for t in range(steps):
            ...
            sequence[t + 1] = self.update_step(
                sequence[t], alpha, S, N, R_cog, R_eff, anchor_type
            )
    ...
    def estimate_lipschitz_numerical(
        self, scenario_params: Dict, psi_grid: np.ndarray = None, eps: float = 1e-5
    ) -> float:
        """
    ...
    def visualize_contraction_analysis(
        self, scenarios: List[Dict], save_path: str = None
    ) -> None:
        ...
        ax1.axhline(
            y=1.0, color="black", linestyle="-", alpha=0.3, label="K = 1 (boundary)"
        )
        ...
        ax3 = axes[1, 0]
        empirical_rates = [
            conv["convergence_rate"] for conv in results["convergence_analysis"]
        ]
        theoretical_rate = results["convergence_analysis"][0]["theoretical_rate"]

        ax3.bar(
            range(len(empirical_rates)), empirical_rates, alpha=0.7, label="Empirical"
        )
...
if __name__ == "__main__":
    ...
    for conv in results["convergence_analysis"]:
        print(
            f"  Scenario {conv['scenario_id']}: rate = {conv['convergence_rate']:.4f}, "
            f"final = {conv['final_value']:.4f}"
        )

    # Create visualization
    psi_updater.visualize_contraction_analysis(
        scenarios, save_path="/Users/ryan_david_oates/Farmer/contraction_analysis.png"
    )
