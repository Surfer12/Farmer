...
class MinimalHybridFunctional:
    ...
    def compute_psi_single(self, x, t):
        ...
        return {
            ...
            'S': S, 'N': N, 'alpha': alpha,
            'hybrid': hybrid,
            'R_cog': R_cog, 'R_eff': R_eff,
            'reg_term': reg_term, 'prob_term': prob_term
        }
...
def numerical_example():
    ...
    responsiveness = "high" if psi_final > 0.6 else "moderate" if psi_final > 0.4 else "low"
    print(f"Step 6: Interpret - Ψ(x) ≈ {psi_final:.2f} indicates {responsiveness} responsiveness")
...
def collaboration_examples():
    ...
    print(f"  Ψ(x) ≈ {O_hybrid_os:.3f} × {exp_term_os:.3f} × {P_adj_os:.3f} ≈ {psi_os:.3f}")
...
def demonstrate_functional_behavior():
    ...
    for name, x, t in scenarios:
        ...
        print(f"  α(t) = {result['alpha']:.3f} ({'symbolic' if result['alpha'] > 0.5 else 'neural'} favored)")
        print(f"  Hybrid = {result['hybrid']:.3f}")
        print(f"  Penalties: R_cog = {result['R_cog']:.3f}, R_eff = {result['R_eff']:.3f}")
...
def generate_simple_data():
    ...
    for x in x_values:
        ...
        print(f"{x:.1f}\t{result['S']:.3f}\t{result['N']:.3f}\t{result['alpha']:.3f}\t{result['psi']:.3f}")
