...
def spatial_distribution_analysis():
    ...
    print(
        ascii_plot(x_values, psi_spatial, "Ψ(x,t=1) vs Position", width=50, height=15)
    )
...
def component_analysis():
    ...
    for name, x, t in scenarios:
        result = functional.compute_psi_single(x, t)
        print(
            f"{name:<15}\t{result['S']:.3f}\t{result['N']:.3f}\t{result['alpha']:.3f}\t"
...
def optimization_landscape():
    ...
    for obj_name, objective_func in scenarios:
        ...
        print(
            f"{obj_name:<15}\t{l1:.2f}\t{l2:.2f}\t{beta:.1f}\t{best_psi:.4f}\t{best_score:.3f}"
        )
...
def collaboration_scenario_analysis():
    ...
    for name, params in scenarios.items():
        ...
        P_total = (
            params["lambda1"] * params["R_cog"] + params["lambda2"] * params["R_eff"]
        )
        ...
        print(
            f"{name:<15}\t{params['S']:.2f}\t{params['N']:.2f}\t{params['alpha']:.1f}\t"
    ...
    for i, (name, psi) in enumerate(results, 1):
        potential = (
            "Excellent"
            if psi > 0.7
            else "Good" if psi > 0.6 else "Moderate" if psi > 0.5 else "Limited"
        )
