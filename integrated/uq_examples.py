...
class HeteroscedasticModel(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int):
        ...
        self.var_head = nn.Sequential(
            nn.Linear(hidden_dim, 1), nn.Softplus()  # Ensure positive variance
        )
...
class ConformalPredictor:
    ...
    def calibrate(self, X_cal: torch.Tensor, y_cal: torch.Tensor):
        ...
        with torch.no_grad():
            if hasattr(self.base_model, "forward") and callable(
                getattr(self.base_model, "forward")
            ):
...
def regression_uq_example():
    ...
    mc_model = MCDropoutModel(
        input_dim=5, hidden_dim=64, output_dim=1, dropout_rate=0.1
    )
    ...
    for method_name, results in methods.items():
        ...
        if results["conf_lower"] is not None:
            ...
            coverage_conformal = interval_coverage(
                y_test_np, conf_lower_np, conf_upper_np
            )
    ...
    for i, (method_name, results) in enumerate(methods.items()):
        ...
        plt.fill_between(
            x_plot, lower_sorted, upper_sorted, alpha=0.3, color="red", label="95% CI"
        )
...
def classification_uq_example():
    ...
    mc_classifier = MCDropoutModel(
        input_dim=10, hidden_dim=64, output_dim=3, dropout_rate=0.1
    )
    ...
    ensemble_ece = expected_calibration_error(
        y_test_np, ensemble_max_prob.detach().numpy()
    )
    ...
    plot_reliability_diagram(
        y_test_np,
        ensemble_max_prob.detach().numpy(),
        title="Deep Ensemble (Temperature Scaled)",
    )

    # MC Dropout reliability diagram
    plt.subplot(1, 2, 2)
    plot_reliability_diagram(
        y_test_np, mc_max_prob.detach().numpy(), title="MC Dropout"
    )

    plt.tight_layout()
    plt.savefig(
        "/workspace/classification_uq_reliability.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        "/workspace/classification_uncertainty_analysis.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.show()
...
def plot_reliability_diagram(
    ...
    sizes = (
        np.array(bin_sizes) * 100 / max(bin_sizes)
        if max(bin_sizes) > 0
        else np.ones(len(bin_sizes))
    )
...
def risk_aware_decision_example():
    ...
    X, y = make_classification(
        n_samples=500, n_features=5, n_classes=2, n_informative=3, random_state=42
    )
    ...
    accuracy_on_predictions = correct_decisions / max(
        1, len(decisions) - sum(1 for d in decisions if d == 2)
    )
    ...
    naive_cost = sum(
        cost_matrix[decision, true_class]
    ...
    for decision_type in range(3):
        ...
        if mask.any():
            plt.hist(
                entropy.numpy()[mask], alpha=0.7, label=labels[decision_type], bins=15
            )
    plt.xlabel("Predictive Entropy")
    plt.ylabel("Count")
