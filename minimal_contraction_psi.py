...
class MinimalContractionConfig:
    ...
    def __init__(self, kappa=0.15, g_max=0.8, L_C=0.0, omega=1.0, 
                 B_max=1.0, lambda1=0.75, lambda2=0.25, beta=1.2):
...
class MinimalContractionPsi:
    ...
    def simulate_sequence(self, psi_0: float, steps: int, 
                         scenario_params: Dict, anchor_type: str = "independent") -> List[float]:
        """
        ...
        for t in range(steps):
            ...
            psi_next = self.update_step(sequence[-1], alpha, S, N, R_cog, R_eff, anchor_type)
    ...
    def estimate_lipschitz_numerical(self, scenario_params: Dict, 
                                   num_points: int = 1001, eps: float = 1e-5) -> float:
        """
    ...
    def print_detailed_analysis(self, scenarios: List[Dict]):
        ...
        for est in results['numerical_estimates']:
            ...
            print(f"    Parameters: α={scenario['alpha']}, S={scenario['S']}, N={scenario['N']}")
            print(f"    Penalties: R_cog={scenario['R_cog']}, R_eff={scenario['R_eff']}")
