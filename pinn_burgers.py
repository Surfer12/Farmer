...
class SimplePINN:
    ...
    def total_loss(self, X_pde, X_ic, u_ic, X_bc, u_bc, 
    ...
    def train_step(self, X_pde, X_ic, u_ic, X_bc, u_bc, 
        ...
        for i in range(len(self.weights)):
            ...
            self.velocities_w[i] = momentum * self.velocities_w[i] - learning_rate * grad_w[i]
            self.velocities_b[i] = momentum * self.velocities_b[i] - learning_rate * grad_b[i]
...
def rk4_burgers(x_grid, t_span, nu=0.01/np.pi):
    ...
    # Solve using RK4
    sol = solve_ivp(burgers_rhs, t_span, u0, method='RK45', 
...
def train_pinn_burgers():
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50 == 0:
            loss, loss_pde, loss_ic, loss_bc = pinn.total_loss(X_pde, X_ic, u_ic, X_bc, u_bc)
            losses.append(loss)
            print(f"Epoch {epoch}: Total Loss = {loss:.6f}, PDE = {loss_pde:.6f}, "
