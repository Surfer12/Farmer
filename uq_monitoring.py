...
class PopulationStabilityIndex:
    ...
    def fit(self, reference_data: np.ndarray):
        ...
        self.reference_bins = np.percentile(reference_data, 
        ...
        self.reference_percentages = (ref_hist + 1e-8) / (ref_hist.sum() + self.n_bins * 1e-8)
    ...
    def calculate_psi(self, current_data: np.ndarray) -> float:
        ...
        # Calculate PSI
        psi = np.sum((current_percentages - self.reference_percentages) * 
...
class KLDivergenceMonitor:
    ...
    def fit(self, reference_data: np.ndarray):
        ...
        self.reference_dist, self.bin_edges = np.histogram(reference_data, 
                                                          bins=self.n_bins, 
                                                          density=True)
    ...
    def calculate_kl_divergence(self, current_data: np.ndarray) -> float:
        ...
        kl_div = np.sum(self.reference_dist * np.log(self.reference_dist / current_dist))
        return kl_div
...
class CalibrationMonitor:
    ...
    def add_predictions(self, y_true: np.ndarray, y_prob: np.ndarray, timestamp: str = None):
...
class UQProductionMonitor:
    ...
    def set_reference_data(self, 
        ...
        if true_labels is not None and prediction_intervals is not None:
            ...
            self.reference_coverage = np.mean((true_labels >= lower) & (true_labels <= upper))
        
        logger.info(f"Reference data set with {len(features)} samples, {features.shape[1]} features")
    ...
    def add_batch(self,
        ...
        for i in range(len(features)):
            ...
            if true_labels is not None and prediction_intervals is not None:
                lower, upper = prediction_intervals
                in_interval = (true_labels[i] >= lower[i]) & (true_labels[i] <= upper[i])
    ...
    def _check_drift_and_alert(self, timestamp: str):
        ...
        for i, monitors in self.feature_monitors.items():
            if i < current_features.shape[1]:
                ...
                kl_score = monitors['kl'].calculate_kl_divergence(current_features[:, i])
        ...
        current_coverage = np.mean(list(self.coverage_buffer)) if self.coverage_buffer else 0
        coverage_drift = abs(current_coverage - self.reference_coverage) if self.reference_coverage else 0
        
        # Check uncertainty drift
        current_uncertainty = np.mean(list(self.uncertainty_buffer))
        ref_uncertainty = np.mean(self.reference_uncertainties) if self.reference_uncertainties is not None else 0
        uncertainty_drift = abs(current_uncertainty - ref_uncertainty) / (ref_uncertainty + 1e-8)
        ...
        if current_ece > self.calibration_threshold:
            alerts.append(DriftAlert(
                ...
                severity="high" if current_ece > 2 * self.calibration_threshold else "medium",
        ...
        for alert in alerts:
            ...
            if (last_alert_time is None or 
                current_time - datetime.fromisoformat(last_alert_time) > timedelta(hours=self.alert_cooldown_hours)):
    ...
    def get_monitoring_dashboard_data(self, hours_back: int = 24) -> Dict:
        ...
        return {
            ...
            'calibration_trend': self.calibration_monitor.get_calibration_trend(hours_back)
        }
    ...
    def plot_monitoring_dashboard(self, hours_back: int = 24, save_path: str = None):
        ...
        fig.suptitle(f'UQ Model Monitoring Dashboard - Last {hours_back} Hours', fontsize=16)
        ...
        if not metrics_df.empty:
            ax1.plot(metrics_df['timestamp'], metrics_df['input_drift_score'], 'b-', linewidth=2)
            ax1.axhline(y=self.psi_threshold, color='r', linestyle='--', alpha=0.7, label='Threshold')
        ...
        ax2 = axes[0, 1]
        if not metrics_df.empty:
            ax2.plot(metrics_df['timestamp'], metrics_df['calibration_error'], 'g-', linewidth=2)
            ax2.axhline(y=self.calibration_threshold, color='r', linestyle='--', alpha=0.7, label='Threshold')
        ...
        ax3 = axes[0, 2]
        if not metrics_df.empty:
            ax3.plot(metrics_df['timestamp'], metrics_df['coverage_rate'], 'purple', linewidth=2)
            if self.reference_coverage:
                ax3.axhline(y=self.reference_coverage, color='orange', linestyle='--', alpha=0.7, label='Reference')
        ...
        ax4 = axes[1, 0]
        if not metrics_df.empty:
            ax4.plot(metrics_df['timestamp'], metrics_df['avg_uncertainty'], 'orange', linewidth=2)
        ...
        if not metrics_df.empty:
            ...
            hourly_counts = metrics_df.set_index('timestamp').resample('1H')['prediction_count'].sum()
            ax5.bar(hourly_counts.index, hourly_counts.values, alpha=0.7, color='skyblue')
    ...
    def export_monitoring_report(self, hours_back: int = 24, output_path: str = None) -> Dict:
        ...
        report = {
            ...
            'metrics_trend': dashboard_data['metrics_history'][-10:] if dashboard_data['metrics_history'] else [],
    ...
    def _generate_recommendations(self, dashboard_data: Dict) -> List[str]:
        ...
        if abs(summary['avg_coverage_rate'] - (self.reference_coverage or 0.9)) > self.coverage_threshold:
            recommendations.append(
...
def simulate_monitoring_scenario():
    ...
    for scenario in scenarios:
        ...
        for batch in range(scenario['n_batches']):
            ...
            if scenario['name'] == "Severe Drift":
                batch_features = np.random.normal(drift, 1 + drift * 0.5, (batch_size, n_features))
            else:
                batch_features = np.random.normal(drift * 0.3, 1, (batch_size, n_features))
            ...
            if scenario['name'] == "Calibration Degradation":
                batch_labels = (batch_predictions.squeeze() > 0.7).astype(int)  # Miscalibrated
            else:
    ...
    print(f"Total Predictions Monitored: {report['summary_statistics']['total_predictions']}")
    print(f"Alerts Generated: {report['summary_statistics']['alert_count']}")
    print(f"Average Input Drift Score: {report['summary_statistics']['avg_input_drift']:.4f}")
    print(f"Average Calibration Error: {report['summary_statistics']['avg_calibration_error']:.4f}")
    print(f"Average Coverage Rate: {report['summary_statistics']['avg_coverage_rate']:.3f}")
    ...
    monitor.plot_monitoring_dashboard(hours_back=48, save_path='/workspace/monitoring_dashboard.png')
